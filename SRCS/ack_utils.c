 ...
 *
 * 2. volatile (keyword in C) tells the compiler not to optimize accesses to this variable,
 ...
 * for acknowledgements (acks). It takes no parameters and simply tells the system:
...
/*
 * install_server_signal_handlers() sets up a robust signal handler for SIGUSR1 and SIGUSR2.
 * 
 * - A sigaction struct 'sa' (defined in signal.h) is used to define how signals are handled.
 * - sa.sa_sigaction = handler sets the handler function (3-argument version) to receive extra info like sender PID.
 * - sa.sa_flags = SA_SIGINFO enables the 3-argument handler mode.
 * - sigemptyset(&sa.sa_mask) ensures no signals are blocked while handling a signal.
 * - sigaction(SIGUSR1, &sa, NULL) and sigaction(SIGUSR2, &sa, NULL) register the same handler for both signals.
 *
 * In Minitalk, SIGUSR1 represents bit 0 and SIGUSR2 represents bit 1. 
 * Using siginfo_t, the server knows which client sent each signal and can send back acknowledgements.
 */
